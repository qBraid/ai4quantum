...
from .circuit import circuit_to_graph_data, qiskit_to_cudaq_kernel
...
def generate_proper_circuits(
...
) -> Tuple[Dict[str, QuantumCircuit], Dict[str, Tuple[Callable, List[List[float]]]]]:
    """
    Generate proper quantum circuits with actual gates using CUDA-Q kernels and Qiskit circuits.
    Args:
    ...
    Returns:
        Tuple containing two dictionaries:
            - circuits_qiskit: Dict[str, QuantumCircuit] mapping circuit names to Qiskit circuits
            - circuits_cudaq: Dict[str, Tuple[Callable, List[List[float]]]] mapping circuit names to
              tuples containing (cudaq_kernel_function, parameter_lists)
    """
    np.random.seed(seed)
    circuits_qiskit = []
    circuits_cudaq = []
    ...
    if 'random' in circuit_types:
        for i in range(num_circuits):
            ...
            # Create CUDA-Q kernel using the same sequence
            random_circuit_kernel = qiskit_to_cudaq_kernel(qiskit_circuit)
            # Store the circuits in their respective dictionaries
            circuits_qiskit.append(qiskit_circuit)
            circuits_cudaq.append(random_circuit_kernel)    # Generate SU2 circuits (similar to hardware-efficient ansatz)
    if 'su2' in circuit_types:
        for i in range(num_circuits):
            ...
            # Create CUDA-Q kernel using the same sequence
            su2_circuit_kernel = qiskit_to_cudaq_kernel(qiskit_circuit)
            # Store the circuits
            circuits_qiskit.append(qiskit_circuit)
            circuits_cudaq.append(su2_circuit_kernel)
    ...
    if 'uccsd' in circuit_types:
        for i in range(num_circuits):
            ...
            uccsd_circuit_kernel = qiskit_to_cudaq_kernel(qiskit_circuit)
            # Store the circuits
            circuits_qiskit.append(qiskit_circuit)
            circuits_cudaq.append(uccsd_circuit_kernel)
    return circuits_qiskit, circuits_cudaq
...
def generate_proper_observables(circuits_qiskit,
                                ...
                                seed      = 42):
    """
    Produces *aligned* observables:
        observables_qiskit – list[SparsePauliOp]   (5-feature extractor)
        observables_cudaq  – list[cudaq.SpinOperator] (for actual simulation)
    """
    np.random.seed(seed)
    obs_qiskit, obs_cudaq = [], []
    ...
    for qc in circuits_qiskit:
        ...
        # CUDA-Q representation ----------------------------------------------
        op = cudaq.SpinOperator(n)            # start with 0-operator
        for pstr, c in zip(paulis, coeffs):
            term = ' '.join(f'{p}{i}'
                            for i,p in enumerate(pstr) if p!='I')
            
            if term:                                # e.g.  term == 'X0 Y1 Z3'
                factors = []
                for tok in term.split():            # ['X0', 'Y1', 'Z3']
                    pauli, idx = tok[0], int(tok[1:])
                    if   pauli == 'X':  factors.append(spin.x(idx))
                    elif pauli == 'Y':  factors.append(spin.y(idx))
                    elif pauli == 'Z':  factors.append(spin.z(idx))
                    # (skip 'I' – multiplying by identity does nothing)

                pauli_term = functools.reduce(operator.mul, factors)
            else:                                   # pure-identity term
                pauli_term = cudaq.SpinOperator(n)

            op += c * pauli_term
        obs_cudaq.append(op)

    return obs_qiskit, obs_cudaq
