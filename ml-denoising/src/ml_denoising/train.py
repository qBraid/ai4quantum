...
def train_mitigator(dataset, model, epochs=100, batch_size=32, lr=0.001, device='cpu', early_stopping_patience=10):
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs}", leave=False):
            ...
            # Model predicts the final value
            corrected_exp = model(circuit_data)
            loss = criterion(corrected_exp, circuit_data.true_exp)
...
def evaluate_mitigator(model, test_dataset, device='cpu', output_prefix="ml_mitigation_output"):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            predicted_correction_val = (corrected_exp - circuit_data.noisy_exp).cpu().numpy().flatten()
