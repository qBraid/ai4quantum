...
def qiskit_to_cudaq_kernel(circ: QuantumCircuit):
    kernel = cudaq.make_kernel()
    qubits = kernel.qalloc(circ.num_qubits)   # qubit register

    for instr in circ.data:
        gate = instr.operation.name
        idx   = [circ.find_bit(q).index for q in instr.qubits]
        qargs = [qubits[i] for i in idx]

        if gate in ('cx', 'cz'):
            getattr(kernel, gate)(qargs[0], qargs[1])
        elif gate in ('h', 'x', 'y', 'z'):
            getattr(kernel, gate)(qargs[0])
        elif gate in ('rx', 'ry', 'rz', 'u1'):
            kernel.__getattribute__(gate)(instr.operation.params[0], qargs[0])
        elif gate == 'u2':
            kernel.u2(*instr.operation.params, qargs[0])
        elif gate == 'u3':
            kernel.u3(*instr.operation.params, qargs[0])
        else:
            raise NotImplementedError(f"{gate} not mapped yet")
